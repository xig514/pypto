...
TypePtr DeduceBlockCreateTileType(const std::vector<ExprPtr>& args,
  ...
  if (blayout >= 0) {
    tile_view.blayout = (blayout == 0) ? TileLayout::none_box : 
                         (blayout == 2) ? TileLayout::col_major : TileLayout::row_major;
  }

  int slayout = GetKwarg<int>(kwargs, "slayout", -1);
  if (slayout >= 0) {
    tile_view.slayout = (slayout == 0) ? TileLayout::none_box : 
                         (slayout == 2) ? TileLayout::col_major : TileLayout::row_major;
  }
  ...
  if (pad >= 0) {
    tile_view.pad = (pad == 1) ? TilePad::zero : (pad == 2) ? TilePad::max : 
                    (pad == 3) ? TilePad::min : TilePad::null;
  }
